(async()=>{
  ...
  const instrumentedSource=source.replace('const app = {','const app = window.__vettaApp = {');
  let bootError=null;
  try{
    new Function(`${instrumentedSource}\nwindow.__vettaApp=app;`)();
  }catch(error){
    bootError=error;
    console.error('Falha parcial ao inicializar o VETTA',error);
  }
  ...
  if(bootError&&!getApp()) throw bootError;
})().catch(error=>{
  console.error(error);
